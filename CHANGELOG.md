...
#### Programs
  ...
  * Add `RNAconsensus` Python program that will replace `refold.pl`
...
#### Library
  ...
  * API: Refactor backtracking implementations and API
  * API: Refactor experimental probing data (SHAPE) implementations
  * API: Refactor auxiliary grammar extension API
  ...
  * API: Add `vrna_string_make_space_for()` and `vrna_string_available_space()` functions in `string.h`
  ...
  * API: Add generic support for experimental probing data via new API in `ViennaRNA/constraints/probing.h`
